...
public class AiChatService {
    ...
    public AiChatService(
            ...
            @Value("${app.query.url}") String queryUrl) {
        this.restClient = restClientBuilder.build();
        this.objectMapper = objectMapper;
        this.queryUrl = queryUrl;
    }

    public String query(String prompt, String authorizationHeader) {
        JsonNode response = restClient.post()
                .uri(queryUrl)
                .header(HttpHeaders.AUTHORIZATION, authorizationHeader)
                .body(Map.of("query", prompt))
                .retrieve()
                .body(JsonNode.class);

        if (response == null || !response.has("result")) {
            throw new IllegalStateException("Query service response does not contain a result");
        }
    ...
    // - product: {tenant_id: string, _id: string, name: string, latest_batch_no:
    // string, remaining_quantity: int, latest_unit_price: double, created_at: date,
    // updated_at: date, schema_version: int}
    // - supplier: {tenant_id: string, _id: string, name: string, email: string,
    // address: string, contact: string, created_at: date, updated_at: date,
    // schema_version: int}
    // - product_batch: {tenant_id: string, _id: string, product_id: string,
    // invoice_no: string, batch_no: string, qty: int, unit_cost: double,
    // unit_price: double, exp: date, created_at: date, updated_at: date,
    // schema_version: int}
    // - invoice: {tenant_id: string, _id: string, invoice_no: string, supplier_id:
    // string, date: date, created_at: date, updated_at: date, schema_version: int}
    // - transaction: {tenant_id: string, _id: string, transaction_id: string,
    // payment_method: string, gross_amount: double, discount_amount: double,
    // net_amount: double, paid_amount: double, balance_amount: double, created_at:
    // date, updated_at: date, schema_version: int}
    // - transaction_item: {tenant_id: string, _id: string, transaction_id: string,
    // product_id: string, qty: int, unit_price: double, created_at: date,
    // updated_at: date, schema_version: int}
    // - audit_log: {tenant_id: string, _id: string, user_id: string, username:
    // string, entity_type: string, entity_id: string, action_type: string,
    // description: string, old_values: object, new_values: object, status: string,
    // error_message: string, ip_address: string, user_agent: string, request_path:
    // string, request_method: string, duration_ms: long, timestamp: date,
    // schema_version: int}
    // - tenant: { _id: string, name: string, sub_domain: string, created_at: date,
    // updated_at: date, schema_version: int}
    // - tenant_config: { _id: string, tenant_id: string, brand: object, ui_theme:
    // object, localization: object, features: object, created_at: date, updated_at:
    // date, schema_version: int}
    // - user: {tenant_id: string, _id: string, username: string, email: string,
    // password: string, roles: array, enabled: bool, created_at: date, updated_at:
    // date, schema_version: int}

    // Allowed response formats (choose one):
    // 1) Find: {"collection":"product", "filter":{...}, "projection":{...},
    // "sort":{...}, "limit":10, "skip":0}
    // 2) Aggregate (for multi-collection joins/lookups, group, etc.):
    // {"collection":"product", "pipeline":[ {"$match":{...}}, {"$lookup":{...}},
    // {"$group":{...}}, {"$sort":{...}}, {"$limit":...} ]}
    ...
    // .defaultSystem("You are an expert in MongoDB. Given the following
    // collections: " +
    // schema +
    // "Convert the user's natural language request into a valid MongoDB JSON query
    // object. " +
    ...
    // } else {
    // throw new IllegalArgumentException("Invalid aggregation stage type: " +
    // stageObj.getClass());
    // }
    // }
    ...
    // safePipeline.add(0, new Document("$match", new Document("tenant_id",
    // tenantId)));
    // } else {
    ...
    // }
    // }
    // throw new IllegalArgumentException("LLM returned invalid JSON query",
    // primary);
    // }
    // }
